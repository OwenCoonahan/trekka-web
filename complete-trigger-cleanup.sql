-- Complete cleanup and recreation of all notification triggers
-- This will remove any lingering old column references

-- Remove ALL notification triggers and functions
DROP TRIGGER IF EXISTS notify_trip_trigger ON trips;
DROP TRIGGER IF EXISTS trip_notification_trigger ON trips;
DROP TRIGGER IF EXISTS notify_trips_trigger ON trips;
DROP TRIGGER IF EXISTS trips_trigger ON trips;

DROP FUNCTION IF EXISTS notify_trip();
DROP FUNCTION IF EXISTS notify_trip_change();
DROP FUNCTION IF EXISTS handle_trip_notification();
DROP FUNCTION IF EXISTS trip_notification();
...
DROP FUNCTION IF EXISTS follow_notification();

-- Now recreate the clean versions
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    -- Notify followers about new trip (using correct column: followed_id)
...
WHERE event_object_table IN ('trips', 'follows');